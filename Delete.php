...
else
{
    ...
    if($resultcheck && $bookresultcheck)
    {
        ...
        while($row=$resultcheck->fetch_assoc())
        {
            ...
            {
                $flag=1;
                echo "<div style='position:relative;top:50%;left:50%;transform:translate(-50%, -50%);color:red;'><center>Book $bookno is been issued by ".$row["Issue_By"]." so it cannot be deleted!!!</center></div>";
            }
        }
    }
    ...
    if($flag==0 && $bookExist)
    {
        ...
        if($result) echo "<div style='position:relative;top:50%;left:50%;transform:translate(-50%, -50%);color:green;'><center>Book $bookno deleted successfully!!!</center></div>";
        else echo $conn->error;
    }
    else if(!$bookExist)
    {
        echo "<div style='position:relative;top:50%;left:50%;transform:translate(-50%, -50%);color:red;'><center>$bookno does not exist in the database</center></div>";
    }
}
?>
